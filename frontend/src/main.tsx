...
type View = "dashboard" | "machines" | "rules";
...
function App() {
  ...
  const reload = () => Promise.all([api.dashboard(), api.machines(), api.rules()]).then(([nextDue, nextMachines, nextRules]) => { setDue(nextDue); setMachines(nextMachines); setRules(nextRules); }).catch((reason) => setError(reason instanceof Error ? reason.message : "Could not reach the API."));
  ...
  return <div className="app-shell">
    ...
    <nav className="tabs">{([["dashboard", "Dashboard"], ["machines", "Machines"], ["rules", "Rules"]] as const).map(([key, label]) => <button className={view === key ? "tab active" : "tab"} onClick={() => setView(key)} key={key}>{label}</button>)}</nav>
    ...
    {view === "machines" && <MachineView machines={machines} onSaved={reload} />}
    {view === "rules" && <RuleView rules={rules} />}
  </div>;
}

function MachineView({ machines, onSaved }: { machines: Machine[]; onSaved: () => Promise<unknown> }) {
  const [editing, setEditing] = useState<Machine | null>(null);
  return <main><section className="section-heading"><div><p className="eyebrow">YOUR FLEET</p><h2>Machines</h2><p>Hard points, service dates, and current odometer readings.</p></div><button className="button primary">+ Add machine</button></section><div className="machine-grid">{machines.map((machine) => <article className="machine-card" key={machine.id}><div className="machine-icon">{machine.type.slice(0, 1)}</div><div><span className="eyebrow">{machine.code}</span><h3>{machine.year} {machine.make} {machine.model}</h3><p>{machine.type} · {machine.status}</p></div><div className="odo"><span>Current odometer</span><strong>{machine.currentOdometer?.toLocaleString() ?? "Not set"}</strong><button className="text-button" onClick={() => setEditing(machine)}>Update reading</button></div></article>)}</div>{editing && <OdometerDialog machine={editing} onClose={() => setEditing(null)} onSaved={async () => { setEditing(null); await onSaved(); }} />}</main>;
}
